...
_Last updated: 2026-08-23 (sixth session, same day) — landing page polish round via the impeccable + animate skills (pushed), then an accuracy experiment (widen Area Championships to all six continents) that regressed and was reverted — see Failed Attempts._
...
2. **If pushing accuracy further**: the easy levers are now applied and mostly exhausted — 2018/2019 label years, real per-athlete profile scraping (dead end, see Failed Attempts), major non-DL meets (Olympics/Worlds/Continental Tour Gold/Euro Champs), and widening Euro-only to all six Area Championships (regression, reverted — see Failed Attempts) are all tried. None of the additive-data levers moved the headline number much (59.1%→60.3%→60.1%, one lever even cost -1.5pts), which at this dataset size (~459 rows) increasingly looks like this feature set's real ceiling rather than an artifact. Remaining honest levers, roughly in order of promise: (a) a per-continent accuracy breakdown before trying Area Championships again piecemeal rather than all-six-at-once, (b) genuinely new predictive features beyond the current 14 (the "add more rows of the same kind" well looks dry — a new feature *type* is more likely to move the number than more data), (c) scraping every meeting worldwide for true full-season per-athlete history (big undertaking, real rate-limit concerns, and no evidence yet it'd help more than the major-meets data already did).
