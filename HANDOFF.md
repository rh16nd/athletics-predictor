...
- `scripts/make-sitemap.py` was re-run so lastmod matches the site (track-insights `701f480`).
- Three properties exist: the `podiumcall.cc` Domain property (the useful one), the www URL-prefix, and `podiumcall.vercel.app`.
- **The biggest SEO gap found: the French site is invisible to Google.** French lives behind a toggle at the same URL with no `/fr/` paths and no `hreflang`, so 974 translated strings can never be indexed or found. Fixing that roughly doubles the indexable surface and is the one substantial SEO task outstanding. After that: links from other sites (the single biggest factor for a domain this new), query-shaped titles on athlete and event pages, and `Person`/`SportsEvent` structured data.

**The formula explainer (updated 2026-09-22).** The Claude artifact "How PodiumCall Works" (https://claude.ai/artifact/UhnBTyVvmPuYgKgbPmTbYe) was rewritten from the code, because version 1 still described 32 Diamond League events, a Random Forest with 14 features and a site that fetched live from the API. Version 2 documents the served model exactly, and doubles as the reference for what the formula now is:
- The served model is `outputs/field_model.json`, experiment `old_marks`, fitted 2026-09-16 on **1,155 finals**, Plackett-Luce (a softmax over the field, maximum likelihood on the order of each final's top three), so a field's podium chances sum to 300%.
- **Eleven signals**, with their fitted weights on standardised features: `fade_gap_third` +0.81, `fade_gap_best` +0.71, `recent_delta` +0.31, `h2h_top` +0.28, `big_podiums` +0.19, `no_history` +0.08, `consistency` 0.00 (the fit put no weight on it), `no_recent` −0.03, `age` −0.02, `sb_prior_season` −0.05, `sb_months` −0.10.
- **The fade rule**: level = this season in full, lifted by at most 0.75 of the gap to an older best that still stands above it, faded 0.35 for last season and 0.3 per year further back (`FADE_LAST_SEASON`, `FADE_PER_YEAR`, `FADE_CAP_SHARE`). Chosen by walking 30 settings over 1,063 finals from 2012 to 2026: **65.3% of medallists against 62.8% for a points ranking**. The unruled baseline scored 66.0% but broke the user's rule, so it was not used. On the 180 Asian finals alone: 64.1% against points' 65.6%, which is why the site says tested level, not better.
- The ship rule per event group (model must beat points on unseen finals, and a shuffled-feature control must fail) is in `field_model.py`'s docstring; a group that fails stays on points and the site labels the method.

**LinkedIn (drafted 2026-09-22, not posted).** Three post drafts in the passion-project voice, plus nine screenshots of the live site at 1440×900 and 1080×2196, were produced in the session scratchpad. The screenshots are regenerable with `scripts/qa`-style playwright runs; the honest numbers to quote are 110 of 172 podium places across two championships (64%), a claimed 60.9%, 87,623 marks from 19,900 competitions since 2008, and 36 events called. The repo's first commit is 12 August 2026, so "six weeks", not "months".

**Repo clean-up, audited 2026-09-22 and NOT yet done.** The user asked for the list; nothing was deleted. On the site: `.tanstack/tmp/…` (a build temp file committed by accident, and `.tanstack/` is not ignored), `bun.lock` + `bunfig.toml` (12 August, while the project builds with npm — two lockfiles from different managers is the one item with real risk, since Vercel can pick bun), `components.json` (shadcn CLI, unused), `.lovable`, `V0-PROMPT.md` + `V0-SAMPLE-DATA.json`, `design/v0/` and `design/intro-video/` (superseded). On the backend: `data/raw/archive.zip` (19.6 MB, the largest thing in the repo, referenced only in a comment and a skip rule — the user's call, since it touches the Render deploy), `outputs/dashboard_2026.html` (tracked but also gitignored, unreferenced), `notebooks/` (one unreferenced file), and loose files that show in every `git status`: `crawl-part1/2.{json,err}`, `.playwright-cli/`, and `data/field/*.csv` including three `_before_*` backups — the three live ones (`finals.csv`, `championship_finals.csv`, `finalist_ids.csv`) are read by `field_data.py` and regenerable, so they belong in `.gitignore` beside `data/worldwide/` and `data/athlete_profiles/`. Keep the twelve `outputs/field_model_*.json`: they are the audit trail the Results page's honesty rests on. Stale remote branches: `v0-redesign` (site) and `v0-redesign-and-depth` (backend); `redesign/terra` is now identical to `main`.

## Earlier (2026-09-22, evening): the redesign goes live
...
**What the user decided (do not reopen)**
- Whole landing page in Terra's style, with our real content only (nothing invented, every number from the API).
- Headline font: a Terra-style serif, on the landing only. Playfair Display (not Instrument Serif or Fraunces, the skill's banned defaults); the rest of the site keeps Space Grotesk and Barlow.
- Accent: our gold, not Terra's pink.
...
**Traps from this session**
- The Bash tool's heredocs mangle backslashes and quotes in Python patch scripts; write the script to the scratchpad and run the file.
...
**State: live**, pushed at the user's word ("yes") on 2026-09-17: backend `8a4230a5`, frontend `b5fadb7` (with the full `build_static_api.py` snapshot, 2,415 files, and the sitemap, 1,115 URLs). Vercel served the new rankings 30 seconds after the push. On www.podiumcall.cc, `scripts/smoke-routes.py` loads 20 of 20 pages in EN and FR, and a fresh browser read the men's 100m on Track (Seville 61.1% model rating), the Asian Games picker in distance order, the dashboard ratings, Josh Hoey's page through the Render API (59.1% model rating) and the 100m event page with no storylines.

**Next**: the Games (23 to 29 September, WA competition `7176091`; the frozen call is never re-frozen), then retrain the championship model with the Asian Games finals and give each championship its own claim on the Results page. French storylines are no longer an open item, since the event pages no longer carry storylines. French country names and search are still open.
...
**Done on 2026-09-16: the rule for old marks, built, tuned and served**

What the user asked for, after they corrected me for reading it as "this season only": old performances still count, but never as much as recent ones, and less the older they are; an athlete breaking records this season must not be beaten by a mark that was good two years ago; the weights come from trial and error on past seasons, with reasons shown. They chose the rule over raw accuracy, for the Asian Games if it was ready by 20 September.

All seven steps are done. What each one produced:

1. **Old marks by season age.** `field_data.earlier_bests` reads the career best before this season, last season's best, the best two seasons ago, and the best from three or more seasons back with the season it came from, which is what lets a mark be faded by its exact age. `attach_scores` (training) and `field_model.serving_rows` (serving) both call it, so the two cannot drift. `finals.csv` was rebuilt: 12,931 rows, every one of the 28 existing columns identical in every row, compared as raw text; the new columns hold a best from two seasons back for 78% of finalists and from three or more for 76%.
2. **The rule in the model.** `faded_level()` reads a level of this season in full (form, the mean of the best five marks), lifted by at most a faded share of an older best that still stands above it. The lift fades with age (`fade_weight`), never stacks across seasons, and is capped at a share of the distance to the best level in the field on this season's marks. That cap is what holds the rule whatever the old mark: an old best can close a gap but never cross it. The features that let history outweigh the season (pb_gap, pb_gap_thin, yoy, breakout_backed) are left out, and `FADE_BOUNDS` holds the signs on the fit as `RECENCY_BOUNDS` does. The settings travel with the model (`fade_of`), so the call fades a mark exactly as the backtest did.
3. **The rule test, as a gate.** `check_old_marks_rule` (`python src/field_model.py --rule-test`) builds 15 made-up finals where A leads on this season, B has a bigger older mark and D is 5 points above B with nothing old. It checks both halves of the rule: the leader is never passed, whatever the old mark and however thin B's season, and a big recent-enough old mark still counts, so a setting cannot pass by ignoring old marks. It checks the level and the win chances, under 20 random fits inside FADE_BOUNDS rather than one fitted model. The model this replaced, `v2_form_best_5`, fails it on 10 of the 15 cases, the first being the user's own example.
4. **Trial and error.** `--old-marks` walked 30 settings over how much last season counts, how fast older seasons fade and how big the cap is, each over every season from 2012 to 2026, each season called only from the seasons before it; three of them are outside the rule on purpose so the record says what the rule costs. Everything is in `outputs/field_model_old_marks.json`. Kept `last35_per30_cap75` (last season 0.35, times 0.3 per further year, cap 0.75): 65.3% of the medallists against 66.0% for the model it replaces and 62.8% for a ranking by points. Old marks counted in full and uncapped named 62.8%, and ignoring them named 64.5%. Every one of those seasons had been used before, so the Asian Games are the first finals no version has seen, and the page says so.
5. **Reasons.** `old_mark_reasons` puts `{personalBest, fromSeason, percent}` on each athlete in the call and on their page, fields rather than a sentence so it reads in both languages. `percent` is the share that survived the cap, so the page never claims more than the model used. The call table shows the line only on rows where an old mark counted; the athlete page always says how the athlete was read. Keys `championship.projection.oldMarks`, `ath.champ.oldMarks`, `ath.champ.seasonOnly` and `ath.champ.seasonOnlyPb`, EN and FR.
6. **Served.** `--refit old_marks` fitted it on all 1,171 finals and replaced `outputs/field_model.json` (the model it replaced is at `outputs/field_model_previous.json`). The Asian Games call and the hammer and 10,000m view were rebuilt with it. In the call the favourite changed in 4 of 36 events (men's 100m, men's hammer, women's 1500m, women's high jump), a podium top three in 11, and the largest move in a podium chance was 31.6 points (Avinash Mukund SABLE 87.5% to 55.9%). An older mark counted for 346 of the 563 called athletes. `backtest_summary` gained an `oldMarks` branch, so the page states this test and not the old one, and `asianGames.how.model`, `rankings.subtitle.field` and `disc.top.disagreeNote` now say how old marks count. The claim that the model was tested on finals it had not seen is gone from those two, because this one was tuned on seasons it had seen.
7. **Step 7 did not apply**, since it was ready before 20 September. Nothing was saved unpublished beside the frozen call.
...
**The Asian Games launched on 2026-09-16**, a week early, at the user's decision once the Ultimate was over. In order: the injury check (10 flags; Neeraj Chopra marked out of the Games from an olympics.com report of 29 August, and not on the entry list anyway; the other nine are Watch flags from Diamond League and Ultimate news), `CURRENT` flipped to `asian-games-2026`, the entry list re-scraped (774 entrants from 41 federations, 36 events called and 14 not; the only failure was the results feed, which cannot answer before the Games), every entrant's season re-fetched (510 of 510) and the call rebuilt (against the committed call no favourite or podium top three changed, and the largest chance move was 2.9 points), then the profiles, the countries (158), a full rankings run with fresh races, the photos, the static snapshot (2,415 files) and the sitemap (1,115 URLs, 597 of them entrants). 699 backend tests, tsc, the build and the smoke test (20 of 20, EN and FR) passed before the push.

**Frozen the same evening, at the user's word** ("push it, and if anything needs updating we update it then"): `freeze_prefinal.py --championship asian-games-2026` froze 36 events and 563 athletes at 2026-09-16T19:15Z, a week before the first session, with no results published. `build_static_api.py --core-only` then changed one published file, `results.json`, which shows the Games as "Nagoya · not run yet · called in advance". That box tells readers the frozen call "cannot be edited now", so **do not re-freeze** (`--force`) before the Games, even with fresher marks: the page would then be saying something untrue. The live call on `/championship` can still be refreshed at any time; the Results page grades the frozen one. During the Games, results come from World Athletics competition `7176091`.
...
**Saved**
- Backend (athletics-predictor): 43 commits ahead of origin before the commit that records the plan for the old-marks rule. The newest are `ed5f59f1` (the old-marks test note), `0c308ba3` (three ways old marks can count, tested on every season), `08e22cfe` (the retrain note), `ee96553b` (the field model retrained with the 2026 finals, and the call rebuilt), `d1934c98` (the 2026 championships in the field model's history, with the check on them), `10fdd862` (the bug pass note) and `b7b60178` (the note on the finished UI pass), and before them `4a0fa4e9` (country pages leave out the Ultimate's places and relays once the Ultimate is over or no longer current). Before that come `a878c19a` (the note that recorded where the session stopped), `d3cdd5ea` (`field_model.py --refit`), `4b02fed1` (the morning's 27 toplists, `standings_detail.json`, `predictions_latest.csv` and the photo focus cache) and `ea8fc2c8` (the race-by-race model in service, with the call and rankings rebuilt).
- Frontend (track-insights-main): 18 commits ahead of origin, up to `c5f5d43`, and still 1 behind `origin/main` (merge it, don't rebase).
- Nothing is pushed in either repo, and `CURRENT` is still `ultimate-2026`.
- Left uncommitted on purpose: `data/countries.json`, rebuilt with `PODIUMCALL_CHAMPIONSHIP=asian-games-2026` to check the country fix. It has 158 nations and 4,867 athletes, none with an Ultimate panel; the committed copy is an older build with 154, 4,797 and 29 nations carrying Ultimate qualifiers. Run order step 4 rebuilds it after the flip, so it belongs in the launch commit. So do two photo caches the bug pass's crawl added entries to (`data/photo_focus_cache.json` 12, `data/wikimedia_photo_cache.json` 18, none removed). The untracked training data in `data/field/` stays out, as before, now with 2026 rows and the `*_before_2026.csv` copies.
...
- Found and left for the user to decide:
  - The landing's hero figures are the Diamond League model's. "60.9% podium hit rate" is the API's `modelAccuracy`. The discipline count beside it now reads 36, fixed in the bug pass below. The Asian Games call comes from the field model, which named 64.4% of medallists on the locked 2024-2025 finals.
...
**Bug pass with playwright-cli (the same night)**
- The user asked for playwright-cli to go through the site and fix the silly bugs. Five browser sessions ran in parallel and made 633 page visits: every internal link from the hubs and event pages (9 hubs, the 36 event pages and 327 athlete pages) plus 4 more hubs and 53 country pages at 1280px in English, then 102 of those pages on an emulated Pixel 10 (360px) and 102 in French. Each page was checked for a failed load, console and network errors, raw translation keys, "undefined" or "NaN", broken images, clipped text, sideways scroll, doubled words, a plural after a 1, and English on French pages. Up to 30 buttons on each of 14 key pages were then pressed at both widths, and seven key pages were screenshotted at 360px (eight images). The scripts lived in that chat's scratchpad; the method is in memory.
- Fixed, in three frontend commits:
  - `0a35d85`: singular wording when a count is 1 (a nation with one athlete or one event, "1 DL point", "1 race across 1 season", and the countdown the day before the Games, which would have read "1 days to Nagoya", checked with the clock set to 22 September in EN and FR); the athlete page's two "no meet history" lines, hard-coded in English; the logo link, which had no name below `sm`; the championship hero, a second `h1`; and the country page's cold-open back link, which said "← All countries" but led to the Performance Index, a page with no country list.
  - `e241ab2`: the country header printed the code twice ("JPN JPN"; `NatFlag` gained `showCode`); the back link touched the breadcrumb; `disc.levelSubtitle` said "the other 32" of 32 finals (now "among the 32 finals"); and the landing counted 32 disciplines where the dashboard counts 36 (it now counts the world rankings, which carry the hammer and the 10,000m).
  - `577a5d3`: `/championship` loaded zoomed out on a phone. The call table's `sr-only` labels are absolutely positioned inside an `overflow-x-auto` scroller that was static, so their containing block was `<main>`, and "Mark from 2025" in a right-hand column made the page 410px wide at 360px. `scrollX` stayed 0, which is why the earlier sweeps passed it; `innerWidth` against `clientWidth` in a `--mobile` session showed it. All seven table scrollers are now `relative`; a fresh phone load of `/championship` stays 360px wide, and a recheck of 11 pages found nothing escaping.
  - Before each commit: `npx tsc --noEmit`, eslint (no new errors; the 9 prettier errors left in `athlete-analytics.tsx`, `field-analysis.tsx` and `qualification.tsx` were already at HEAD), locale keys 911 each with the same set, `npm run build`, and `scripts/smoke-routes.py` (20 of 20).
- Checked and fine: no page failed to load, no raw keys, no "undefined" or "NaN", no broken images, and no sideways scroll at 1280px. No button press left a console error or a dialog that Escape would not close. The one popover off screen, the win chance tip on `/championship` at 360px, came from the zoom bug fixed in `577a5d3` (the tip clamps to `innerWidth`, which read 410); the phone button test was not re-run after the fix. Keyboard search opens an athlete, and a country's "Show all" and filter work. "Gout GOUT" on `/country/AUS` is the sprinter's real name.
- Found and left for the user:
  - The storylines on each Diamond League event page stay in English on French pages ("9 wins from 17 real career meetings", "the biggest real in-season gain among the top contenders", the injury note). `build_storylines` in api.py writes the sentences, and only each story's title is translated, by its type; en.ts says computed storylines stay out of the locale files. Translating them means the API sending each story's numbers and names as fields and the page building the sentence, a design change for the user.
  - Opening an athlete with no Diamond League profile (the 10,000m and hammer athletes, for instance) logs a 404 in the console: `useAthleteProfile` asks for the profile before the status endpoint, by design. Readers see nothing, but a smoke test that included such an athlete would fail on it.

**Old marks, tested on every season (2026-09-16), as the user asked**
- The user asked for the model to lean less on old marks, since athletes are beating them this season, and for one approach across the site, explained simply. They chose to test three versions on every past season and keep the one naming the most medallists; to have one model for the whole site with a plain explanation; and to settle the model before launch and change the site after the Games. The plan: `C:\Users\rayen\.claude\plans\do-you-agree-that-vast-moore.md`.
- The test (`0c308ba3`; `python src/field_model.py --all-seasons` writes `outputs/field_model_all_seasons.json`). `ALL_SEASON_CANDIDATES` are today's `v2_form_best_5`, `recency_form_best_5` (old marks count at most half, and not at all once beaten) and `season_only` (no career best or last season, except last season as the stand-in when there is no mark this season). All three read races the same way and are one model for every event. The rule was fixed before they ran: most medallists, then most winners named first, then log-likelihood.
- The result on 1,063 finals from 2012 to 2026, each season called only from the seasons before it: today's model named 66.0% of medallists and 571 winners, recent over old 65.6% and 551, this season only 65.1% and 529, and points 62.8% and 510. On the 180 Asian finals: 65.2%, 65.2% and 64.6%, against 65.6% for points. Only on the 57 finals of 2026 did this season only lead (72.0% against 70.2%). Today's model was kept, so nothing was refit and no pick in the call changed.
- The Asian Games page's method panel now states this test (`asianGames.how.testAllSeasons`, frontend `c5f5d43`; `backtest_summary` returns `method: "allSeasons"` when the report kept the served model), including that those seasons also chose it. Checked with playwright-cli in EN and FR at 1280 and 360px; 682 backend tests, the frontend build and the smoke test (20 of 20) pass.
- Every season has now been used for testing or training, so the Asian Games are the next finals no version has seen.
- After the Games, from 30 September, comes Phase B of the plan: one model for the whole site. The field model's top-20 view for all 36 events replaces the Diamond League random forest on Track and Field, the athlete page figure, dashboard favourites and the landing's hit rate, and How it works explains that one model in plain words.

**Retrained with the 2026 finals (2026-09-16), as the user asked**
- The user asked to retrain the model on the new version. The served model was already `v2_form_best_5`, fitted on every final from 2009 to 2025, so a refit alone would have changed nothing. The user chose to add the 2026 Diamond League Final and the 2026 Ultimate first: 57 finals, 5% more.
- Checked once before they joined (`d1934c98`, `outputs/field_model_check_2026.json`), with both models fitted on 2009-2025 only. On the Diamond League Final (31 of its 32 finals could be scored) `v2_form_best_5` named 64 of 93 medallists and 19 winners, the season-best model 61 and 18, and points 63. On the Ultimate it named 54 of 75 and 15 winners, as the night before. Both together: 118 of 168 medallists and 34 winners, against 115 and 30 for the season-best model and 118 for points.
- How they joined (`d1934c98`): `field_data.dl_finals` also reads `data/dl_final_2026_results.csv` (`DL_RESULTS_LATER`), so `data/dl_final_results.csv` stays the Diamond League model's label file, untouched, and the 2026 cut-off comes from World Athletics' listing of the Final (Brussels, 4 September). `ultimate_finals` reads `data/ultimate/event.json`. 2026 finals are scored with this season's world list (`attach_all_scores`), and `--ids` takes `--years`. The run: `field_data.py --ids --years 2026`, the 2026 seasons copied from `data/field/races_2026` into `data/field/races` (then `--races --years 2026` had nothing to fetch), `--report`, and `field_model.py --refit`. Every pre-2026 row of `finals.csv` and `finalist_ids.csv` was checked identical to the `*_before_2026.csv` copies, and both 2026 championships were scored 100% and trained on.
- The result (`ee96553b`): 1,171 finals from 2009 to 2026. No weight moved more than 0.016 (pb_gap 0.381 to 0.365, recent_delta 0.205 to 0.213, h2h_top 0.240 to 0.247), and this season's signals now carry about 3.3 times the weight of history, against 3.2 before. The replaced model is `outputs/field_model_previous.json`. The experiment and its locked-year record carry over, so the page still states the 2024-2025 test.
- Rebuilt with it: the Asian Games call (no favourite and no podium top three changed in the 36 events; podium chances moved at most 2.2 points, in the men's shot put) and the hammer and 10,000m in `data/world_rankings.json` (top threes unchanged, at most 0.7 points), with `world_rankings.py --only`. A full `world_rankings.py` run also rewrote the 32 Diamond League events from the morning's predictions, because their committed entries predate `modelKind: "form"`; that was set aside, since launch step 4 runs it in full.
- Retrain again once the Asian Games have been graded: add them to the history the same way (their results as finals rows in `all_finals`), then `--refit`.
...
- The site is at `https://www.podiumcall.cc` (Vercel), the API at `https://podiumcall.onrender.com`. Live since 2026-09-16 is the old-marks model, with the static snapshot rebuilt from it: backend `f2d4b765`, frontend `f30bd09`. The championship on the site is the Asian Games since 2026-09-16, launched a week early.
...
6. Frontend: merge `origin/main`; regenerate the sitemap with `PODIUMCALL_BASE_URL=https://www.podiumcall.cc python scripts/make-sitemap.py` (since 2026-09-16 it reads the snapshot step 5 just wrote, not the API, so it lists exactly what is published, the championship's entrants included); `tsc`, eslint on changed files and `npm run build`; then `scripts/smoke-routes.py` against the dev server with the predictor's venv (20 pages, EN and FR).
7. Commit both repos, with the refreshed toplists, and push by 21 September. Check the live site about a minute later.
8. Before the first session on 23 September: re-run step 3, then `python src/freeze_prefinal.py --championship asian-games-2026`, rebuild, and push. If the old-marks rule version was not ready by 20 September, also save its predictions, unpublished, beside the frozen call, to grade both after the Games.
...
The model:
- The locked years 2024-2025 are used, and since the retrain of 2026-09-16 so are the 2026 Diamond League Final and the Ultimate. Any further change to the field model needs finals it has not been tested on: the Asian Games, once graded.
...
- **Step 2, built 2026-09-14, committed locally and not pushed:**
  - **The field is the official entry list.** The organisers' results site runs on an API at `back.results.asiangames2026.org`, and `/s/AG2026/en/ATH/entries/list` already lists 774 athletes from 41 federations with their events. Each federation's page, `/entries/org/{code}`, adds the World Athletics id. Replies are a zlib stream sent as text and decode with `zlib.decompress(body.decode("utf-8").encode("latin-1"))`. The planned stand-in field (top 30, two per nation) was never needed.
  - `src/asian_games_scraper.py` reads the entries and pages the Asian area toplists (`?regionType=area&region=asia`, now joined with `&page=`). It writes `data/asian_games_2026/event.json`, `asia/{key}_2026.csv` and the merged snapshot `raw/{key}_2026.csv`. A non-Asian nationality fails the scrape, and a run that lost the field or results is not saved. `--results-only` is what the refresh button runs.
  - A matching bug turned up on the first real run. Area toplists link profiles as `/athletes/<country>/<name>-<id>` and the id reader only knew `athlete=<id>`, so 283 entries went unmatched. After the fix, plus a name-parts match for entries without an id, 179 are unmatched, mostly athletes with no 2026 mark in the Asian top 300.
  - `src/asian_games_predictions.py` applies the 6-of-8 rule and the 5% floor below: 6 events by the model, 26 on points, 18 not called (six relays, and events we hold no data for). Each event records why it went to points, too few records or a favourite under the floor, and the page says which. An athlete on no world toplist links to World Athletics rather than to a page that cannot load.
  - `build_2026_features` takes an optional `snapshot_path`, `fetch_results` a `competition_id`, and the injury checker follows the registry with a `--championship` flag. `/api/championship/summary` serves the nav and the countdowns.
  - 569 backend tests pass, up from 521.
- **Step 3, built 2026-09-14, committed locally and not pushed:**
  - `/championship` renders whichever championship is current, and `/ultimate` redirects to it. The Ultimate's page body moved unchanged to `components/dl/ultimate-body.tsx`.
  - `src/lib/championship-themes.ts` holds the three themes with each ratio beside its value. `Shell` takes a `PageGround` with its own `blooms` and `trackCurve`.
  - The Asian Games page has a green ground; a hero with the OCA red sun as a solid disc and the emblem's purple, gold and green line running into it; a bar showing how the 50 events are called; "How each event is called"; the call per event (a chance column where the model called it, points where it did not); and the events without a call.
  - The nav tab, dashboard band, landing badge and schedule marquee read the summary. Each Results box wears its theme through CSS variables scoped to the box.
  - EN and FR have 856 keys each. `tsc` and `vite build` pass. ESLint finds no errors in the changed files; the 14 errors it reports in `src` are in files this work did not touch.
- **Contrast, measured on the rendered page:** Asian Games ground text 4.66, hero 7.32, panels 5.18; Results boxes 6.18 (Diamond League) and 8.24 (Ultimate), gradient badges included. Two traps on the way:
  - Red laid over green composites to brown, so the sun is a shape in the hero, not a bloom on the page.
  - The podium place badge paints `--primary-foreground` on a gradient from `--terracotta` to `--gold-strong`. White text failed at the Ultimate's gold end (1.75), hence its light violet fill with dark text. A scan that reads only `backgroundColor` misses gradients and flags the wrong box.

**Settled by the user on 2026-09-14**
1. **A 5% floor.** With the history rule alone, 5 of the 12 model events had a favourite under 1.5% (women's triple jump: 0.4%, 0.3%, 0.2%). The model learned on world-class finals, so those were world-scale numbers, not chances in an Asian field. An event now also needs the model's favourite at 5% or more, or it is ranked on points. The model keeps the men's 110m hurdles, pole vault, long jump and javelin, and the women's steeplechase and javelin.
...
**Done in the third session (2026-09-14): the user's three items**

At the start the user settled three questions: the hammer and the 10,000m go on the whole site, the colours they could not read were the Results page boxes, and the Asian Games entrants with no page on the site should get one.

1. The Results page boxes.
   - The cause: every `Panel` paints a cream `card-surface` gradient as a background image, and a championship box only swapped the flat `--card` colour underneath it. Light text sat on cream at about 1.1:1. The second session's measurement read `backgroundColor` and never saw the image.
   - `card-surface` in `styles.css` now reads `var(--card-wash, <the gradient>)`, and `box()` in `championship-themes.ts` sets `--card-wash: none`.
   - The selected pill's fill was lifted so its text clears 7:1: Diamond League `#44a2d9` to `#5bb0e0` (6.18 to 7.27), Asian Games `#f18a76` to `#f39382` (6.99 to 7.53).
   - Measured on the rendered page with gradients composited, all 63 text elements in each box clear 7:1. The lowest is 7.08 in the Diamond League box (muted text on a chip) and 8.24 in the Ultimate's (the place badges). The Asian Games box renders only after its freeze; computed from its tokens, its lowest is 7.53.
2. The hammer throw and the 10,000m, on the whole site, ranked on points only.
   - One list says they have no model: `POINTS_ONLY_DISCIPLINES` in `src/feature_builder.py`. Neither is a Diamond League event, so there is no meetings log, no 2008-2025 history and no row of them in the training set, which is unchanged.
   - Added to `live_fetcher.DISCIPLINE_URLS`, `api.DISC_LABELS` (the 10,000m is in `MIDDLE_DISTANCE`, so marks format as mm:ss), the `FIELD_EVENTS` sets, `WA_EVENT_TO_KEY` (World Athletics writes "10,000 Metres"), both `EVENT_CODES` maps, the EN/FR discipline names, `seo.ts` and the athlete page's field-event set.
   - `python src/live_fetcher.py --only men_HT,women_HT,men_10000m,women_10000m` fetches just those toplists and leaves the Diamond League standings alone. Each came back with 100 rows.
   - `world_rankings.py` never runs the model on them. It sends `modelAvailable: false` with an empty `model` list, and the Track/Field table shows points with no toggle and no rating column. `world_rankings.json` has 36 disciplines and `countries.json` 400 more athlete rows.
   - Two things would have broken and were fixed. Country pages linked every discipline to `/discipline/<key>`, which 404s for these four; those rows now link to `/field?disc=` or `/track?disc=`. And a hammer athlete's status page fell through to "outside the projected top 6"; it now says `reasonCode: points_only`. The static build no longer asks for their discipline pages.
   - Checked in the browser: `/field?disc=men_HT` ranks Bence HALÁSZ first at 84.25 with no toggle or rating column, the Hungary page links its hammer rows to the Field page, and Halász's own page loads.
   - Their athlete pages show no season form, because `data/athlete_profiles/` is only fetched for the model's athletes. That is what every other non-predicted athlete's page shows too. Fetching profiles for their top 50 would fix it.

3. Why some entrants got no prediction, and pages for the ones that had none.
   - Why: the entry list gave 156 of 599 entries no World Athletics id, so they could only be matched to the Asian toplist by name, and 23 more had an id but sat below the top 300 in Asia.
   - `complete_matches()` in `src/asian_games_scraper.py` now looks each unplaced entrant up at World Athletics directly. An entry with no id goes through WA's own athlete search (`searchCompetitors`), accepted only when exactly one result has the same nation, sex and date of birth and shares a name. If that id is on the Asian list, the row is used. If not, the season best comes from the athlete's profile: the best legal outdoor 2026 result in the event, from `getSingleCompetitor.resultsByYear`. A profile's `resultScore` is the toplist's Results Score (checked on two entrants: 1169 and 1066 in both places).
   - Three traps turned up on real data, each now has a test. The two lists can swap which name is the family name: the Mongolian entry "DAVAANYAM Myagmarsuren" is given name Dawaanyam at WA. The Asian and world toplists can spell one athlete two ways ("Taepoong NAM", "Tae-poong NAM"), and the snapshot keeps the world row, so an entrant whose id is on the world list now takes the world spelling (`world_spelling()`). And a single-named entrant came out as "Sangay .".
   - Result on the original 32 events: 502 of 599 entries ranked, up from 420, and 97 unranked, down from 179. Of the 82 newly ranked, 39 were found through the search and 43 have a mark from their profile. Of the 97 left, 55 have a WA profile with no 2026 result in that event and 42 match no profile. Across all 36 events, 540 of 658 entries are ranked, and 32 people match no profile, 8 of them from Qatar.
   - Each event lists its unranked entrants as rows at the foot of its table under "Entered, not ranked", each with its reason, instead of a sentence of names.
   - Pages: `api.py` reads a championship's merged snapshot after the world toplist (`season_snapshot_paths()`), so all 540 ranked entrants have a page, against 138 of 419 before. The search index includes them, with no world rank. An athlete page opens with the current championship's call on them (`championship_call()`, shown until the championship ends). `python src/athlete_profile_scraper.py --championship asian-games-2026` fetches their profiles, which fill the season form and career blocks. The static build writes a status page for each of them (`championship_pairs()`).
   - Two calls changed because of this, not because of a rule. The men's 400m hurdles went to the model once the search found Abderrahman SAMBA, who had no id on the entry list, which put him in the top 8 by points and made 6 of the 8 with a record (favourite: Samba, 10.3%). The women's shot put went once Yusun JEONG took the world spelling her record is filed under (6 of 8, favourite 12.7%). The call is now 8 events by the model, 28 on points and 14 not called. Tae-poong NAM, unscored before, is 8th in the men's javelin.

**Still open from those items**
- An unranked entrant now takes World Athletics' spelling too. That change came after the last scrape, so until the next one a Mongolian entrant reads "Dawaanyam MYAGMARSUREN" in the 1500m and "Myagmarsuren DAVAANYAM" in the 800m.
- World hammer and 10,000m athletes have no season form on their pages (item 2).
- The landing page's meta description in `routes/index.tsx` still ends "Next up: the Ultimate Championship in Budapest". Change it with the flip.
...
**Watch out:** `refresh-results.cmd` pushes every local commit, and the backend is 6 commits ahead of GitHub (`91d2c275`, `e6092a69`, `d5421302`, `3ba51442`, `91e91fe4`, and the one that wrote this note) with the frontend 2 ahead (`1a39e09`, `7e6c9e1`). While `CURRENT` is the Ultimate, the button refreshes the Ultimate. `data/wikimedia_photo_cache.json` and `data/photo_focus_cache.json` were touched by the local API during checks and are not part of this work.
...
_Last updated: 2026-09-02. **PodiumCall IS LIVE.** Frontend on **Vercel** (`https://podiumcall.vercel.app`), API on **Render** (`https://podiumcall.onrender.com`), both auto-deploying from `main`. The full launch record and the four launch bugs are in **item 27** and in memory `project_launch_2026_09_02`. The pre-launch hardening that made it possible is items 24-26. The earlier eighteenth/nineteenth-session notes (retrain, AA contrast, ARIA, verifiability) are all resolved and recorded below._

_Last updated: 2026-09-01 (**eighteenth session**). **The head-to-head data was rebuilt: 0o's open half is closed, and four more scraper defects turned up while doing it, including a missing 2025 season and a 400m hurdles that did not exist.** Read Next Steps **18**; one decision is open there (retraining the model on it). Everything below this line is the seventeenth session's note and still stands._
...
**Phase 1 is DONE as of 2026-08-31, and LIVE since 2026-09-02.** The site is built, redesigned, refreshed, deployed and merged to `main`, and the Final it was built for is 4–5 September. Phase 2 (item 10) is the next real body of work and its gate lifts the moment the Final is run.

**Reaching more people is now part of the goal, not a side quest (added 2026-09-03).** The user asked for the app in French and Arabic, Arabic being both the most-wanted and the harder of the two. **French is finished and live**; Arabic is next and is a different kind of job, because it is a right-to-left language and the layout has to mirror, not just the words change. This sits alongside Phase 2 rather than replacing it: a system that predicts any major championship is worth more if it can be read by more of the people who follow athletics. See item **30** for where the translation stands and exactly what Arabic needs.
...
> | # | Item | Where |
> |---|---|---|
> | 1 | **Arabic translation + RTL.** The agreed next step. French is done and live; Arabic needs a mirrored layout, not just a string table, and should be read by a native speaker before it ships | item 30 |
> | 1a | **Phase 2: generalise beyond the DL.** The gate was "not before 4 Sep" and it lifts on its own | item 10 |
...
> | P | **Membership / subscription (a fan-funding model). PARKED 2026-09-05** after a brainstorm. Not active work; the good ideas (a technical data-depth tier, wind + exposed feature sheet as the anchor) are written up so they are not lost | item 31 |
>
...
20. **Same session, the design arc, the landing hero was rebuilt around the track, then audited by a dual-agent critique. `DESIGN.md` now exists.**
    ...
    **20c. Critique findings deliberately NOT actioned, with reasons.** The user was asked and chose:
    ...
    - ~~Still open from the critique: error recovery **1/4** and Help/Documentation **1/4**~~ **BOTH CLOSED 2026-09-01**. See items 22c and 23. The snapshot is at `.impeccable/critique/` (gitignored), re-run `/impeccable critique` to regenerate.
...
23. **Same session, the critique's other 1/4, and it was not the one it looked like.** Help and Documentation scored 1 with a specific complaint worth quoting: *"this page argues for a model's credibility. **Zero external links.** `waUrl` sits unused in the payload."* It is about **verifiability, not vocabulary**, the landing named World Athletics as its source **nine times** and gave the reader nothing to click, while `waUrl` sat in three payload types and rendered in exactly one place, the athlete page.

    **A FAQ page was rejected earlier and stays rejected.** The site already explains its numbers where they are shown, every panel on the discipline page carries an explanatory subtitle, Qualifying has "How to read it", the stats page explains the depth ladder and the WA points axis. Reading them confirms the earlier decision was right. What the site could not do was let anyone **check** those numbers.

    **What shipped:** the landing podium's three plaques each link to that athlete on World Athletics (the season best printed above it is exactly what can be checked there); "Data scraped from World Athletics" is now a link in `Shell`'s footer, the one claim made on all nine pages, and again in the landing's own footer, which renders under `<Outlet />` and so never inherited `Shell`'s; likewise the scrape claim above the pipeline. **Landing: 0 external links → 5**, all with `rel="noopener noreferrer"` and an aria-label naming who is being verified.

    **Deliberately NOT per athlete row** in the discipline/dashboard tables. Those names already link to that athlete's own page, which carries the World Athletics link with room to explain it. Repeating it per row is link soup competing with the internal navigation the tables exist for.

    **The interesting bug, and it is the same one as 22b's athlete-page note.** A shared link component cannot have one hover colour. Darkening to `--foreground` is right on a light card and measured **3.51** on the terracotta, against the link's own **4.65 at rest**, i.e. *a hover that makes a link harder to read than not hovering it*. Hence a `tone` prop. Its canvas value resolves to **plain white (5.30)**, deliberately **not `--landing-fg`**, which was the natural choice and is wrong: that variable is **scoped to `.landing`**, while `Shell`'s footer sits on the same terracotta *outside* that scope, where it would have resolved to nothing and failed silently. Same trap 20d records from the other direction.

    **A note on the "zero external links" finding:** it was true of the landing, which is the file that was critiqued. The dashboard already had external links via the news feed's letsrun.com items.

24. **Same session, a 17-point launch checklist, audited, and the two "failures" it looked like having were mostly measurement error.** Six items already passed (custom 404, no broken links, favicon, no placeholder text, no unused nav, no horizontal scroll). Four shipped. The rest are gated on one thing.

    **Two corrections that matter more than the fixes, both traps in this environment:**
    - **There is NO horizontal scroll, on any page.** A reading of "47px overflow" on the discipline page was an artefact: the Browser pane pins `window.innerWidth` to its own width regardless of the emulated viewport, so `scrollWidth - clientWidth` returns exactly `innerWidth - clientWidth` every time, 47px at an emulated 375, 2px at 420. The page never actually scrolls (`scrollTo(300)` leaves `scrollX` at 0). **Check `actuallyScrolls`, never `scrollWidth`, in this pane.** A fix had already been written and was reverted.
    - **The site has ZERO WCAG 2.5.8 violations and had zero before.** Raw "height < 24px" flagged 10 targets on the dashboard and 27 on Qualifying; **the criterion's spacing exception clears all of them** (nearest target centres 35–50px apart, against a 24px requirement), and the 14px ones are inline in a sentence, which it excludes outright. Applying 24x24 without the exception produces a false failure list on every data table on the site.

    **Shipped (track-insights `2ff364b`):** the 404 got a `<main>`, its own title and `noindex` (it renders outside `Shell`, same reason the landing lacked a main); the breadcrumb became `nav > ol > li` with a real link home and `aria-current` on the leaf, geometry unchanged at 300x19; **JSON-LD exists now**, `WebSite` at the root, `BreadcrumbList` per page, and degrades rather than lies, omitting `url` and withholding the breadcrumb entirely until `VITE_SITE_URL` is set, since Google rejects a trail of bare names. Verified in both states. The dashboard's confidence rows moved their padding onto the link (target 21.8 -> 45.8px, whole row clickable, panel height identical), a UX change, explicitly **not** a conformance fix.

    **Still open, and four of them are ONE blocker, a real domain:** privacy policy, cookie consent, analytics, Search Console, plus sitemap.xml and canonical tags. Cookie consent is *correctly* absent today: nothing sets a cookie or tracks, so there is nothing to consent to, it becomes mandatory the moment analytics lands. Also open: the **22 distinct font sizes against DESIGN.md's 6 declared roles** (the critique's 102 findings, one rule), and the landing's below-the-fold structure reading generic. Case studies and a FAQ remain deliberately not-done, **re-confirmed by reading the pages**: every discipline panel explains itself in its subtitle, Qualifying has "How to read it", stats explains the depth ladder.

25. **Same session, accessibility, audited properly the second time.** The first pass checked `lang`, landmarks, headings, alt text, contrast and target size, found them fine, and stopped. Going further found **four real ARIA defects**, none visible, all now fixed (track-insights `458299a`).

    - **The search combobox was silent to a screen reader while arrowing through results.** Focus correctly stays in the input (that IS the pattern), so the only way to announce the current option is `aria-activedescendant`, and there was none, the visual highlight had no spoken equivalent. Options now carry ids; verified live that ArrowDown moves it `opt-0` → `opt-1` with `aria-selected` following.
    - **Each option wrapped a `<button>`.** An element with `role="option"` must not contain a focusable control, it put every result in the tab order, competing with the arrow keys the input already owns. The `<li>` is the option now. **Both activation paths re-verified end to end after the restructure**: mouse click and keyboard Enter each navigate to the right athlete.
    - **`aria-controls` dangled.** Set unconditionally while the listbox only renders when open, so closed it referenced a non-existent id.
    - **The trajectory chart's data points claimed `role="button"` and nothing happens on Enter or Space.** They are focusable so a keyboard user can reach each point and have `onFocus` surface the mouse's tooltip, kept, that part is good, but the role promised an action. Now `role="img"`. **The focus ring was checked BEFORE changing it**: `styles.css` targets `[tabindex]:not([tabindex="-1"])`, not the role, so it survives. Changing a role is a classic way to silently delete a focus indicator.
    - Two `<nav>` landmarks, one unnamed and so indistinguishable in a landmark list. Top nav is now "Main".

    **Measured clean, recorded so it is not re-audited:** 7 `:focus-visible` rules with **zero `outline: none` anywhere**, no positive tabindex, skip link resolving to `<main id="main">`, no duplicate ids, no autoplaying media, every data table carrying a `<caption>` and `scope` on all its `<th>` (22 and 6) with real row headers on the h2h matrix, reflow clean at **320px** with the wide table scrolling in its own container as 1.4.10 permits, and the marquee honouring `prefers-reduced-motion`.

    **On target size, the checklist answer needs its correction repeating:** the site has **zero WCAG 2.5.8 violations and had zero before** any of this. Raw `height < 24` flags 10 targets on the dashboard and 27 on Qualifying; the criterion's **spacing exception** clears all of them (nearest centres 35–50px against a 24px requirement), and inline-in-a-sentence links are excluded outright. Applying 24×24 without the exception produces a false failure list on every data table on this site.

26. **Same session, pre-launch security review + hardening, and the launch path.** A full pre-launch pass on the REAL app (a Strix install + a deliberately-vulnerable practice target came first, as a warm-up; that is all in scratchpad, not the repo). The honest finding: the app's attack surface is small, read-only public data, no accounts, no cookies, no SQL, no shell, no writes, React auto-escapes, so this is production **configuration**, with one genuinely critical item.

    **Shipped, both repos on `main` and pushed**, athletics-predictor `prelaunch-hardening`, track-insights `3233754`:
    - **`app.run(debug=True)` → secure by default.** `python api.py` now runs **waitress** (real WSGI, debugger OFF); dev opts into Flask's reload+debugger with `PODIUMCALL_DEBUG=1`, which must NEVER be set on a public host (the Werkzeug console = RCE on any error). This was the one critical finding. `PODIUMCALL_HOST`/`PORT` are env-configurable; falls back to Flask (debug off) if waitress is missing. **waitress added to requirements.txt.**
    - **CORS pinned** to an origin allow-list (`PODIUMCALL_CORS_ORIGINS` for prod, localhost defaults for dev) instead of the wildcard `CORS(app)`.
    - **`disc_key` validated** against `DISC_LABELS` before file reads in `/api/athlete/<disc_key>/<name>` (the discipline/projections routes were already guarded via their prediction lookup; athlete-status already validated). The URL router blocks `/` so real traversal was mostly stopped, this is defense-in-depth.
    - **API security headers** (`nosniff`, `X-Frame-Options`, `Referrer-Policy`) via `after_request`; client error messages no longer name internal files.
    - **Frontend security headers**, `public/_headers` (Cloudflare) sets X-Frame-Options/nosniff/Referrer-Policy/HSTS/Permissions-Policy/COOP on every route, verified to survive the nitro build. **CSP is written but COMMENTED**, it needs the real domain + API origin (a guess would block the app's own fonts/photos/API); two-step enable documented in the file.
    - **Search Console + sitemap**: `__root.tsx` renders a `google-site-verification` meta ONLY when `VITE_GOOGLE_SITE_VERIFICATION` is set (verified both ways live); `make-sitemap.py` now also writes the robots.txt `Sitemap:` line from `PODIUMCALL_BASE_URL` and takes `PODIUMCALL_API`. Dry-run wrote 276 URLs (7 fixed, 32 disciplines, 237 athletes); fake-domain artifacts reverted.

    Verified: `Server: waitress` (not Werkzeug), the headers present, `/api/predictions` 200, `/api/athlete/EVIL/x` → 404 "unknown discipline", **339 tests green**, api tests 47/47, frontend tsc/lint/build clean.

    **The launch path, as advised:** the code is launch-ready; what remains is the user's. (1) **You do NOT need to buy a domain to launch**, Vercel's `*.vercel.app` / Cloudflare's `*.pages.dev` give a free HTTPS origin that works as `VITE_SITE_URL`, in the CSP, in the sitemap, and as the Search Console property; add a custom domain later. This unblocks the whole SEO/CSP chain. (2) **Frontend** → Vercel (needs `NITRO_PRESET=vercel` and headers moved to `vercel.json`, since `_headers` is Cloudflare-only) or stay on Cloudflare (zero migration). (3) **API** → a persistent host (Render/Railway/Fly), NOT serverless, the model pkl + pandas + per-request CSV reads make cold starts and bundle limits a poor fit; set `PODIUMCALL_CORS_ORIGINS`, keep `PODIUMCALL_DEBUG` unset; scrape on your machine and upload `outputs/` + the model. (4) Then set `VITE_API_BASE_URL`/`VITE_SITE_URL`, fill+uncomment the CSP, generate+submit the sitemap, verify in Search Console.

    **Local `:5000` note:** a zombie socket from a pre-session api.py held the port and resisted taskkill (clears on reboot); the hardened server was verified on alt ports (5055/5057) instead. Not code-related, not launch-relevant.

27. **2026-09-02, LAUNCHED. The site is public and verified live end to end.** `track-insights` → **Vercel** (`podiumcall.vercel.app`), `athletics-predictor` API → **Render** (`podiumcall.onrender.com`). Both auto-deploy from GitHub `main`. Full detail + config in memory `project_launch_2026_09_02`; the essentials and the bugs:

    **The four launch bugs, in order (all fixed):**
    - **`NITRO_PRESET=versel`**, a typo in the Vercel env var (s→c) failed the build (`Nitro entry is missing! Is "versel" preset correct?`). Reproduced locally exactly. Fix: `vercel`. **Note for next time:** the `nitro: { preset: "vercel" }` the user added to `vite.config.ts` is IGNORED by the `@lovable` wrapper (config-only build → `cloudflare-module`); the **env var is the only thing that sets the preset.**
    - **Frontend built but every `/api/*` returned 404** ("no 2026 toplists", "Predictions are not available yet") while `/api/health` was 200. Cause: the runtime-data commit `0dc6c4b8` ("Add runtime data for hosted deploy", 133 files ~42MB, force-added past `.gitignore`) was **local-only, never pushed**, Render had code+model but no data. Fix: push it. **This is the standing data-refresh model:** the API serves committed data, never scrapes; to update predictions, scrape locally → `git add -f` → commit → **PUSH** → Render redeploys.
    - **Search Console meta tag wouldn't verify**, the tag was genuinely absent from the served HTML. `VITE_GOOGLE_SITE_VERIFICATION` must carry the `VITE_` prefix, be set for **Production**, and needs a fresh redeploy (VITE_ vars bake in at build time). Resolved; property verified; sitemap (276 URLs) submitted.
    - **CSP**, added to `vercel.json` (NOT `public/_headers`, which is Cloudflare-only and Vercel ignores). Rolled out **report-only first**, verified on the live site by injecting a `securitypolicyviolation` listener and probing real origins + a disallowed `evil.example.com` control (control fired → detector proven), then **enforced**. Final live check under enforcement: dashboard data loads, Space Grotesk loads, athlete photo loads, zero real-origin violations.

    **Live config, for reference:** Vercel, `NITRO_PRESET=vercel`, `VITE_API_BASE_URL=https://podiumcall.onrender.com`, `VITE_SITE_URL=https://podiumcall.vercel.app`, `VITE_GOOGLE_SITE_VERIFICATION`, headers+CSP in `vercel.json`. Render, start `PODIUMCALL_HOST=0.0.0.0 PODIUMCALL_PORT=$PORT python api.py` (waitress, debug off), `PODIUMCALL_CORS_ORIGINS=https://podiumcall.vercel.app`, `PODIUMCALL_DEBUG` unset; free tier ~30s cold start.

    **The CSP is a whitelist**, a new external origin (analytics, new font/image host, moved API) must be added to the `Content-Security-Policy` in `vercel.json` or the browser blocks it. First thing to check if a resource mysteriously stops loading: the browser console names the directive + blocked URL.

    **Custom domain is the only thing NOT done** and it is optional, the site launched on the free `*.vercel.app` / `*.onrender.com` origins. Adding one later means: point DNS, then update `VITE_SITE_URL`, `PODIUMCALL_CORS_ORIGINS`, the CSP `connect-src`, and re-run `make-sitemap.py`.

28. **2026-09-02, external feedback triaged; a real "How it works" page built, four UI polish items parked as NEXT STEPS.** A reviewer's feedback table was checked against the actual site: several items were **already built** (dashboard search, discipline hover explanations, athlete honours/medals/rankings, the computed Storylines as "key takeaways"), and the recurring **FAQ** request was resolved the way the no-FAQ decision allows, a single discoverable **`/how-it-works`** route (in the nav + footer) that consolidates the two questions reviewers keep asking (accuracy, data sourcing). Its numbers are read **live from the API** (`modelAccuracy`, `modelAccuracyToplist`, `corpus`), so a retrain updates the page on its own; the prose renders even before the API answers.

    **NEXT STEPS, four UI polish wins the user approved but asked to defer (not yet built):**
    - **Metric tooltips**, hover explanations on "Top-3 Hit Rate" (dashboard), and "Projected" / "Podium chance" (track/field table headers, `discipline-table.tsx`). Extend the existing `title=` pattern already used across the components. Highest value-per-effort.
    - **Color-code the podium-chance column**, a subtle high→low scale on the probability cell in `discipline-table.tsx` so likely medallists pop.
    - **Flag icons by nationality**, a flag next to the 3-letter `a.nat` code on the track/field tables. Needs an **IOC-code → flag** map (IOC "GBR" ≠ ISO "GB", so emoji regional-indicator flags need a ~50-entry lookup).
    - **Stats chart contrast**, the Performance Index legend + axis ticks use `text-muted-foreground` (`depth-ladder.tsx`); measure it on the canvas and bump if it fails AA (compositing method, per [[feedback-browser-pane-measurement-traps]]).

    **Deliberately NOT actioned, with reasons:** an athlete prose *bio* (we only have WA's structured honours, inventing bio text is exactly what this project refuses); landing "add data-viz / less sparse" (a real change to a deliberately-composed page, offered, not chosen); landing text contrast (measured AA-clean this session).

29. **2026-09-03, the post-launch feedback rounds (all shipped + live on `track-insights` `main`), and in-app translation started, French first.**

    **Shipped since item 28** (each pushed; full detail in the memory files named):
    - The **four parked UI-polish items** are DONE + live: metric tooltips (a drawn info glyph + a tap-and-hover `InfoTip`, portalled out of the table's `overflow-x-auto` so it isn't clipped, not just `title=`), **field-relative** podium-chance colour (per-event probabilities aren't cross-comparable), **70 vendored SVG flags** (emoji render as letters on Windows; IOC→ISO2 map in `src/lib/flags.ts`), and stats-contrast measured **7.91:1 (AAA)** so left alone. See `project_ui_polish_batch_2026_09_02`.
    - **Mobile round**: first-run **welcome modal** + a persistent About button (`welcome-modal.tsx`, localStorage `podiumcall:welcome:v1`), the tooltips made tap-friendly, and nav/panel spacing fixes. See `project_mobile_onboarding_2026_09_02`.
    - **How-it-works redesigned boxless** (one cream sheet, hairline-ruled sections, inline figures instead of stat tiles), **removed from the top nav** (it's an explainer, so footer + welcome dialog only), and **search added to mobile** (a magnifier toggles a full-width row).
    - **"Make the explanations concrete"**: sharpened the tooltips + how-it-works with plain-language specifics, but NOT false precision, the podium `%` is not a calibrated frequency so no "4-in-10-races" gloss.
    - **Athlete + discipline pages**: decimal age → whole number in the hero; **PB gap now carries a unit** (`s` for track, `m` for field); **info buttons** on the stats people kept asking about (PB gap, WA score, Spread, World top-100 median, Field scored, a "how to read this grid" on the head-to-head matrix, and each "What separates them" column); and the **"Every pairing" grid spacing** (names were congested).
    - **Season-best bar** recoloured to a bright, ringed **gold-light→gold-strong** so it stands out from the normal terracotta→brick bars (a lightness jump, so it reads in greyscale and for colour-blind viewers), in `athlete-analytics.tsx`.
    - The **humanizer** was swept across every remaining page (per [[feedback-humanize-prose]]), and applied to **both READMEs and this `HANDOFF.md`** along the way.

    **In-app translation started here and is now FINISHED for French, see item 30 for the completed state.** What follows is the plan as it stood when the foundation landed, kept because it records why the layer is shaped the way it is. The **foundation is LIVE and working**: `src/lib/i18n.tsx` (`useT`/`t`, `{{var}}` interpolation, localStorage `podiumcall:lang`, sets `<html lang>` AND `dir`), string tables in `src/lib/locales/{en,fr}.ts` (en is the source of truth, a missing fr key falls back to en), and an **EN/FR switcher** in the nav. **English is the default and completely unchanged; French is opt-in.** Translated so far: the **CHROME only** (nav, footer, search, welcome modal). To continue:
    - **Finish French, page by page.** Start with the DASHBOARD and HOW-IT-WORKS (the two most-used), then the rest. Translate a whole page at a time so no page reads half-French. **Caveat**: a sentence with inline `<b>` or a link can't pass through `t()` as one string, split it into prefix/suffix keys (done for the footer: `footer.scrapedFrom` + `<WaSourceLink/>` + `footer.notAffiliated`) or add a tiny `**bold**` renderer. The dashboard/how-it-works prose is full of inline `<b>`, so budget for that. API-driven content (discipline/athlete names, computed verdicts/storylines) stays in its scraped English.
    - **Then Arabic + RTL** (the hard half). Add `"ar"` to `Lang`/`LANGS` with `dir:"rtl"`, an `ar.ts`, then the layout sweep: physical→logical CSS (`ml/pl/text-left` → `ms/ps/text-start`), and mirror the nav, tables and charts. The `dir` hook is already wired, so the flip itself is one line; the layout is the work. The switcher will likely need to become a dropdown (3 languages; the mobile nav is already tight). **Get the Arabic read by a native speaker before it goes public**, athletics terms and the careful explanations don't machine-translate cleanly.

    **THE FINAL GOAL is unchanged, and its gate is about to lift** (see **Goal** at the top, and Next Steps **10**): a prediction system for **any major athletics competition**, not just the Diamond League. Phase 1 (the 2026 DL Final, Brussels **4–5 September**) is the proving ground, not the destination. **Phase 2, generalise beyond the DL, opens the moment the Final is run** (today is 2026-09-03, so that is imminent). The championship podium labels it needs are already scraped (~669 rows, roughly doubling the training set). Do NOT start Phase 2 before the Final; do NOT resume model-accuracy work without asking first (it is ceilinged and well-evidenced, see 0p/0q and `project_ceiling_and_data_quality`).

30. **2026-09-03 (later), the French translation is COMPLETE and live, and Arabic is the next step.**

    **What shipped.** Every page body, not just the chrome: all ten routes (landing, dashboard, track, field, discipline, athlete, qualification, stats, schedule, how-it-works), the 404, every component, and **all 32 discipline names**. **603 keys at exact EN/FR parity**, checked with a key-set diff rather than by eye. **English is the default and byte-identical to what shipped before**, verified by reading the same pages back in English after each round. Merged to `main` and pushed, so it is live. Nine commits, one per page group, plus the merge.

    **The patterns that made it possible, and that Arabic should reuse:**
    - **`src/lib/rich-text.tsx`**, a small `<Rich text={t(k)} />` renderer for `**bold**` and `*italic*` inside a translated sentence. The alternative was splitting every paragraph into three keys around each `<b>`, which is miserable to translate because word order moves between languages and the fragments stop being sentences. How-it-works leans on this heavily.
    - **Fixed-set API strings are translated on their KEY, never by matching the English.** The depth verdict (`verdict.key`), storyline titles (`storyline.type`), meet statuses and now the 32 discipline names (`discKey`, via `discName()` in `dl-data.ts`) all arrive from Python as English but carry a stable key beside them. No backend change was needed for any of it.
    - **Sentences Python composes are rebuilt on the client from their parts.** The near-miss "why they're not in the projected field" reason was English prose; the API also sends `reasonCode` and the `dl` standings row, so `notInFieldReason()` reassembles it in the reader's language. Verified byte-identical to the API's own wording in English. The same trick is available for anything else the API writes.
    - **The French discipline names are all built on MASCULINE head nouns** ("le 100 m", "le saut en hauteur", "le lancer du poids", "le triple saut"). About a dozen sentences embed a discipline behind an article, so this keeps all 32 grammatical with no per-name article and no gender field. **Keep that property if the names are ever edited**, or those sentences break.
    - `startNoun`/`startVerb` became `startNounKey`/`startVerbKey`, and `ordinal` became `ordinalIn(lang, n)` because French writes "5e" and "1er" where English writes "5th". English-only word arrays indexed by a count were deleted rather than translated.
    - **The trap that bit five times: `.map((t) => …)` shadows the translate function.** Renamed to `tr`/`tier`/`tick`/`c` in the depth ladder, trajectory chart, athlete analytics, field analysis and the landing ticker. Grep for `(t)` before adding `useT` to a file.

    **Deliberately left in English, all three correct:**
    - The **root error boundary** in `__root.tsx`. It renders in place of the root component, which is where `I18nProvider` is mounted, so `useT()` would throw exactly when that screen is most needed. There is a comment in the file saying so.
    - **Athlete names** and the **storyline sentence** (`text`), which Python composes with live numbers. Storyline *titles* are translated, because those are a fixed set.
    - `pageHead()` SEO and browser-tab titles.

    **Two real bugs fixed here, both reported from use, neither found by review:**
    - **The top bar overlapped itself in French.** It switched to a `1fr auto 1fr` grid on `sm+` to centre the nav in the viewport, which pinned the right-hand cluster to the logo's width. English fits; French does not. Measured at 1440px: the search input rendered 224px inside a 139px slot and sat on top of the EN/FR switcher, and the timestamp wrapped to two lines in a 64px bar. Now one grid shape at every width, the search width moved off the input onto its wrapper so it shrinks instead of spilling, and the timestamp never wraps.
    - **The mobile nav had part of itself where scrolling could not reach**, which is what Android users were reporting as "the top bar doesn't show everything". `justify-content: center` on a scrollable flex row pushes the overflow past the START edge; `scrollLeft` clamps at 0 and cannot get there, and neither can `scrollIntoView`. Measured 226px of nav stranded at 375px, including the active page's own link. Fixed with `justify-content: safe center` in `styles.css`. **It is in the stylesheet on purpose**: written as a Tailwind arbitrary class it compiled to nothing, the computed value came back `normal`, and the nav *behaved correctly anyway* because `normal` acts as `flex-start`. A fix that does nothing and passes its own test is worth remembering. See traps 4-6 in `feedback_browser_pane_measurement_traps`.

    **Also fixed:** the landing page had **no language switcher at all**, which mattered because it is the page a first-time visitor actually lands on. The switcher lives in `TopNav`, and the landing renders its own header. Added, along with its header CTA, which was still hardcoded English. On a phone that CTA is 178px, longer in French, and with the switcher beside it the brand clipped to "PODIUMCA", so it is hidden below `sm` where the hero repeats the identical button anyway.

    **ARABIC, THE NEXT STEP. This is the harder half and it is a layout job, not a translation job.**
    - Add `"ar"` to `Lang`/`LANGS` in `src/lib/i18n.tsx` with `dir: "rtl"`, and an `src/lib/locales/ar.ts` mirroring `en.ts`. **The `dir` hook is already wired and already sets `<html dir>`**, so the flip itself is one line. Everything below is the actual work.
    - **The RTL layout sweep.** Physical CSS has to become logical: `ml-*`/`mr-*` → `ms-*`/`me-*`, `pl-*`/`pr-*` → `ps-*`/`pe-*`, `text-left`/`text-right` → `text-start`/`text-end`, and `left-`/`right-` positioning reviewed case by case. Then the things that mirror by hand: the nav, every table, the charts (a season timeline should still run in reading order), the podium steps, and any arrow or chevron glyph that points somewhere.
    - **Numbers and marks stay left-to-right inside RTL text.** Times, distances and dates are the whole point of this site, and getting bidirectional runs wrong makes "9.82" render as nonsense. Budget real checking time for this specifically.
    - **The switcher becomes three segments and probably should become a dropdown.** The top bar was measured and tuned for exactly two; a third segment puts it back into the overlap this session just fixed.
    - **Have the Arabic read by a native speaker before it goes public.** Athletics vocabulary and this site's careful hedging ("chance of a podium, not of winning") do not machine-translate cleanly, and the whole product rests on being precise about what it claims.
    - Reuse everything in the patterns list above. The discipline names, verdicts, storyline titles and the rebuilt reason sentence are all key-driven already, so Arabic gets them by adding strings, with no component changes.

31. **2026-09-05, membership / subscription as a FAN-FUNDING model. PARKED by the user after a brainstorm, written down here so the good ideas survive.**

    **The decision that frames everything.** The user's own words: "I want fans to fund the thing I love building." So this is a supporter model, not an edge-selling one and not B2B data licensing. Two consequences settle most of the design. The predictions stay free forever: they are the product and the entire top of funnel, and paywalling them betrays the fan-funding intent. And a good perk is one a non-supporter does not feel punished for missing. Pricing and tiers were left undiscussed on purpose; the user asked to stop talking about "how much" and focus on what to offer.

    **The offer the user actually wants: a technical / data-depth tier.** It is for the fan who wants the underlying data and to draw their own conclusions, leaning less on the single predicted number the free app showcases. Ingredients and the recipe, not the cooked meal. Most of this data already exists. It is collected to feed the model and then hidden behind one number, so the tier is mostly surfacing what is already there, which makes it cheap.

    The pieces worth keeping, roughly best-first by how distinctive and how cheap they are given the data on hand:
    - **Wind is the sleeper.** The WIND column is already on every scraped mark, and `wind_adj_season_best` already exists as a feature. Legal versus wind-aided splits, a wind-adjusted "true level", the best legal mark of the season. Serious sprint and jumps fans care about this a lot and almost nobody surfaces it well.
    - **The full feature sheet, exposed and sortable.** Everything the model sees, in a table instead of buried behind the prediction: season best, career best, pb_gap, form and recency, days_since_last, meets_count, gap_variability, DL points, h2h_win_rate. Sort the field by consistency or freshness and reach your own conclusion. This is the core of the tier.
    - **Round-by-round (heat, semi, final).** The `Pos` field already encodes it (2h4, 1f1, 3sf3). Who fades in finals and who peaks in them. Pure derivation from data on hand.
    - **Athlete comparison.** Pick two to four athletes, every metric side by side, full head-to-head, progression curves overlaid.
    - **Full result log per athlete.** Every mark with wind, venue, round, date and WA score.
    - **Export (CSV), and maybe a personal API key.** The truest expression of "here is the data". CSVs are already produced, so export is nearly free.
    - **Model transparency as the capstone.** Feature importances per discipline, calibration and hit-rate by event, the honest backtest. For someone relying less on the prediction, telling them exactly how much to trust it is the most respectful perk, and it fits the site's honesty-as-identity.

    **Anchor if it gets built: wind plus the exposed, sortable, exportable feature sheet.** Distinctive, cheap because the data exists, and the purest hit of "rely on the data, not our pick".

    **Two cautions recorded so nobody rediscovers them the hard way:**
    - **This tier is exactly what a model-builder or a bettor wants** (full features, h2h, wind, export). The user set the edge-chasing audience aside and chose fan-funding, but the product is the same either way. Decide whether you are comfortable being the free data source under someone else's betting spreadsheet, and market and caveat it accordingly.
    - **Data people are merciless about errors.** The known artefacts stop being quirks and become broken promises the moment the raw data itself is the product: the toplist-depth thing (2 of 32 toplists are 500 rows, not 100) and the per-discipline probabilities that do not sum comparably (they rank athletes within one event only). Clean and caveat those before any data tier ships.

    Lighter supporter perks also discussed, worth keeping as the wrapper around the data tier: follow-athletes plus alerts (the strongest engagement hook, and it fights within-season churn), a supporter newsletter or dev log in the founder's voice (this is the actual funding relationship, and it works in the off-season), roadmap votes and athlete/photo requests (ties perks to the real backlog), and cheap status (a supporter badge, a name on a wall, a custom accent theme, founding-member flair). The seasonality problem, athletics goes quiet October to April, argues for an annual "back the season" pass or a founding-member one-time payment over a monthly rental.
