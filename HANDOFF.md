...
**Backtest accuracy: 60.2%** (56/93, 2023 holdout, train on 2021-2022, 31 of 32 disciplines). Honest progression: 46.2% (fake, bug-inflated) → 43.6% (real baseline) → 53.8% (historical data rework) → 59.0% (h2h fix, 13 disciplines) → 58.1% (31 disciplines) → **60.2%** (fixed a wrong `women_400h` 2023 label — see below). Every step isolated and verified before deploying.
...
- **8 DL_RESULTS entries are genuinely absent from the scraped seasonal toplists**, confirmed via substring search (not a spelling issue): Dafne Schippers (women_200m, 2022), Emmanuel Korir (men_800m, 2023 test year), Yomif Kejelcha (men_1500m, 2023 test year), Angelica Bengtsson (women_PV, 2022), Katie Nageotte (women_PV, 2023 test year), Juan Miguel Echevarria (men_LJ, 2022), Berihu Aregawi + Jacob Krop (men_5000m, 2021). For elite/famous athletes like these, absence from a top-100 seasonal toplist is surprising — most likely a real gap in what `historical_scraper.py`'s single-page scrape captures for that specific discipline+year, not that they genuinely ranked outside the top 100. **Worth re-checking each one the way the Sydney McLaughlin entry was re-checked** (see above) before assuming it's a scraping gap — that one turned out to be a wrong label, not a missing-data problem. Not investigated further this session — flagging honestly rather than silently accepting it, since 4 of these are 2023 test-year labels that put an artificial ceiling on that discipline's backtest score.
